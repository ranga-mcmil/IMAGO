...
import { deleteProductCategory, updateProductCategory, type Category } from "@/app/actions/category-actions"
...
interface ClientProductCategoriesTableProps {
  categories: Category[]
  page: number
  perPage: number
...
export function ClientProductCategoriesTable({
  categories,
  page = 1,
  perPage = 10,
  totalCategories,
}: ClientProductCategoriesTableProps) {
    ...
    <>
      ...
      <div className="flex items-center justify-between border-t px-4 py-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href={`/products/categories?page=${Math.max(1, page - 1)}`} passHref>
            <Button variant="outline" size="sm" disabled={page <= 1} className="h-7 px-2 text-xs">
              Prev
            </Button>
          </Link>
          <Link href={`/products/categories?page=${Math.min(totalPages, page + 1)}`} passHref>
